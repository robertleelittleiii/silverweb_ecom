...
class UpdatePermissionsCart < ActiveRecord::Migration[5.0]
  def self.up
    ...
    right = Right.create name: '*Access to all Cart actions', controller: 'cart', action: '*'
  ...
  def self.down
    # Destroy all rights
    right = Right.find_by_name('*Access to all Cart actions')
    begin
      right.destroy
    rescue StandardError
      puts('image_libraries right not found.')
    end
  end
end
