...
class OrdersController < ApplicationController
  ...
  before_action :force_ssl, only: [:enter_order]
  ...
  def force_ssl(options = {})
    ...
    if request.ssl? || Rails.env.development?
      ...
      redirect_options[:host] = host if host
      flash.keep
      redirect_to(redirect_options) && return
    end
  end
  ...
  def enter_order_square
      ...
      if @cart.total_price > 0
        if @order.save
          ...
          if @order.purchase(@cart)
            ...
            if !Settings.order_notification
    ...
    respond_to do |format|
      if error_occured
        puts('****************** ERROR OCCURED ************')
        puts(@order.errors.inspect.to_s)
  ...
  def enter_order
    ...
    if params[:order].blank?
      ...
      @order.cc_expires = begin
                            Date.new(params[:order]['cc_expires(1i)'].to_i, params[:order]['cc_expires(2i)'].to_i, params[:order]['cc_expires(2i)'].to_i)
                          rescue StandardError
                            nil
                          end
      @order.cc_number = params[:order][:cc_number] || ''.dup
      @order.cc_verification = params[:order][:cc_verification] || ''.dup
      @order.nonce = params[:nonce] || ''.dup

      puts("-=-=-=-=-=-=-=-=-=-=-=-=-=->>>> #{@order.inspect}")

      if params[:save_billing_addresses] == '1'
      ...
      if params[:save_shipping_addresses] == '1'
  ...
  def invoice_list_report
    ...
    @orders_ids = Order.joins(:transactions).where(order_transactions: { success: true }).where("orders.created_at >= '#{@start_date}' and orders.created_at <= '#{Date.parse(@end_date)}'").select('id').collect(&:id)
  ...
  def resend_invoice
    ...
    if Settings.order_notification != 'true'
  ...
  def product_list_report
    ...
    render 'product_list_report.html', layout: 'print_landscape.html'
  end
  ...
  def customer_sales_report
    ...
    @orders_ids = Order.includes(:transactions).where(order_transactions: { success: true }).where("orders.created_at >= '#{@start_date}' and orders.created_at <= '#{Date.parse(@end_date)}'").select('id').collect(&:id)
  ...
  def conditions_user(params = {})
    ...
    if params[:search][:value]
      conditions << "(orders.id LIKE '%#{params[:search][:value]}%' OR
            ...
            orders.created_at LIKE '%#{params[:search][:value]}%')"
    end
    conditions.join(' AND ')
  end
