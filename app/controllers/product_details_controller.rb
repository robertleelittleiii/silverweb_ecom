...
head :ok
  ...
  def create_empty_record
    @product_detail = ProductDetail.new
    @product_detail.product_id = begin
                                   params[:product_id]
...
head :ok
  ...
  def total_objects(params = {})
    @total_objects = ProductDetail.where(product_id: [params[:product_id]]).where(conditions).count
  end

  def datatable_columns(column_id)
    case column_id.to_i
    when 0
      'product_details.sku_active'
    when 1
      'product_details.inventory_key'
    when 2
      'product_details.size'
    when 3
      'product_details.color'
    when 4
      'product_details.units_in_stock'
    else
      'product_details.units_on_order'
    end
  end
