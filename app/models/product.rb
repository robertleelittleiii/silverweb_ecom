...
class Product < ActiveRecord::Base
  ...
  def price
    if discount_available
      msrp.to_f - ((discount.to_f / 100) * msrp.to_f)
    else
      msrp
    end
  end
  ...
  def primary_image
    pictures.where(title: 'Primary').first
  end

  def product_list_image
    pictures.where(title: 'Product List').first
  end

  def slider_image
    pictures.where(title: 'Slider').first
  end

  def next_in_collection
    product_list = Product.tagged_with([category[0].name], on: :category).order(:position)
    product_list.index(self)
    if product_list.last == self
