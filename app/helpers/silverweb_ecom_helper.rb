...
module SilverwebEcomHelper
  ...
  def ecomm_attributes
    ...
    return_val  << " <div id='logged-in'>#{!User.find_by_id(session[:user_id]).blank?}</div>"
    return_val  << ''
    return_val  << '</div>'

    return_val.html_safe
    end

  def page_data_attr_ecom(addtional_script)
    "<div id='data-reload' class='hidden-item' data-page-params='#{request.original_url}' data-page-update='call_document_ready(\"#{controller_name + '_' + action_name}\")' data-additional='#{controller_name + '_' + addtional_script.sub('-', '_')}'></div>".html_safe
    end

  def product_data_attr(template_name = ''.dup)
    "<div id='data-reload' class='hidden-item' data-page-params='/site/product_detail?id=#{@product.id}' data-page-update='site_product_detail', data-additional='#{template_name}'></div>".html_safe
    end
end
